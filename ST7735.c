...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6, y*10, *pt, ST7735_BLACK, ST7735_MAGENTA, 1);
    pt++;
    x = x+1;
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_BLACK,ST7735_MAGENTA, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
      ...
      int32_t    currentx = x1;
        ...
        if ( abs(delx) > abs(dely) ){
            ...
            while (currentx != x2){
                ...
                currentx += sign;
                currenty = (y1 * delx + sign * count * dely) / delx;
                count++;
            }
     }
     else {
            ...
            while (currenty != y2){
                ST7735_DrawPixel(currentx, currenty, color);
                currenty += sign;
                currentx = (x1 * dely + sign * count * delx) / dely;
                count++;
            }
     }
}
